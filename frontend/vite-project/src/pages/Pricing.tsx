    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <main className="max-w-6xl mx-auto px-8 py-16">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-semibold text-[#1d1d1f] mb-4">Выберите тариф</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`bg-white rounded-2xl p-8 shadow-[0_8px_30pxrgb(0,0,0,0.08)] ${
                plan.current ? 'ring-2 ring-[#0071e3]' : ''
              }`}
            >
              <h3 className="text-xl font-semibold text-[#1d1d1f] mb-2">{plan.name}</h3>
              <div className="mb-6">
                <span className="text-4xl font-bold text-[#1d1d1f]">{plan.price}</span>
                <span className="text-[#6e6e73] ml-1">₽ {plan.period}</span>
              </div>
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-2 text-[#6e6e73]">
              ...
              <button
                className={`w-full py-3 rounded-xl font-medium transition-all duration-200 ${
                  plan.current
                    ...
                    : 'bg-[#0071e3] text-white hover:bg-[#0077ED] shadow-[0_4px_12pxrgb(0,113,227,0.3)]'
                }`}
